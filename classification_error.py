...
def gaussian(x,mean,sd):
    return np.exp(-0.5*np.power(x-mean,2.0)/(sd*sd))/(sd*np.sqrt(2*np.pi))

def integrand(x,mean,sd,cr):
    return 0.016*np.exp(0.5*0.204*0.204)*np.power(2*x*cr,2.013)*gaussian(x,mean,sd)

def aboveGroundBiomass(mean,sd,cr):
    return integrate.quad(lambda x: integrand(x,mean,sd,cr),0,np.inf)
    ...
    for pc in pctCorrect:
        ...
        for m in meanVec:
            ...
            for s in sdVec:
                ...
                correctAGB, err   = aboveGroundBiomass(m,s,1.0)
                if correctAGB == 0:
                    print(str(m)+", "+str(s))

                for ns in range(nSamples):
                    #print("Mean: " + str(m) + ", Std: " + str(s) + ", samples:  " str(ns) + " out of " + str(nSamples))
                    x = rng.random()
                    if x<p:
                    ...
                    agb, errEst = aboveGroundBiomass(hMean,hSd,1.0)
                    estimatedAGB[ns] = agb
